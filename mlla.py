...
class LinearAttention(nn.Module):
    r""" Linear Attention with LePE (without RoPE).
...
class MLLAAttentionForKAT(nn.Module):
    def __init__(self, dim,  num_heads, input_resolution=None, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, **kwargs):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
    ...
    def forward(self, x):
        ...
        act_res = self.act(self.act_proj(x))
        x = self.in_proj(x).view(B, H, W, C)
        x = self.act(self.dwc(x.permute(0, 3, 1, 2))).permute(0, 2, 3, 1).view(B, N, C)
