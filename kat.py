...
from mlla import MLLAAttentionForKAT
...
class Block(nn.Module):
    def __init__(
            ...
            dim: int,
            num_heads: int,
            input_resolution: Tuple[int, int] = None,  # 新增参数
            mlp_ratio: float = 4.,
            qkv_bias: bool = False,
            qk_norm: bool = False,
            proj_drop: float = 0.,
        ...
        super().__init__()
        self.input_resolution = input_resolution  # 存储分辨率
        ...
        # # 将 Attention 替换为 MLLABlock
        # self.attn = MLLAAttentionForKAT(
        #     dim=dim,
        #     input_resolution=input_resolution,
        #     num_heads=num_heads,
        #     mlp_ratio=mlp_ratio,
        #     qkv_bias=qkv_bias,
        #     drop=proj_drop,  # 使用 proj_drop 作为 drop rate
        #     drop_path=drop_path,
        #     norm_layer=norm_layer,
        # )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 动态更新 attention 的分辨率
        if hasattr(self, 'input_resolution') and self.input_resolution is not None:
            self.attn.input_resolution = self.input_resolution
...
class KATVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        #         dim=embed_dim,
        #         num_heads=num_heads,
        #         input_resolution=self.patch_grid,  # 传递分辨率
        #         mlp_ratio=mlp_ratio,
        #         qkv_bias=qkv_bias,
        #         qk_norm=qk_norm,
        #         init_values=init_values,
    ...
    def forward_features(self, x: torch.Tensor, H: int = None, W: int = None) -> torch.Tensor:
        # 如果传入了 H, W，使用它们；否则使用动态计算
        if H is not None and W is not None:
            # 存储当前的分辨率
            self.current_resolution = (H, W)
        else:
            # 原有的动态计算逻辑
            B, C, H, W = x.shape
            self.current_resolution = (H, W)
        ...
        for block in self.blocks:
            if hasattr(block, 'attn') and hasattr(block.attn, 'input_resolution'):
                block.attn.input_resolution = self.current_resolution
            if hasattr(block, 'input_resolution'):
                block.input_resolution = self.current_resolution
